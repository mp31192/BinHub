...
class ConvCapsuleLayer(layers.Layer):
    ...
    def call(self, input_tensor, training=None):
        ...
        conv = K.conv3d(input_tensor_reshaped, self.W, (self.strides, self.strides),
...
class DeconvCapsuleLayer(layers.Layer):
    ...
    def call(self, input_tensor, training=None):
        ...
        input_shape = K.shape(input_transposed)
        input_tensor_reshaped = K.reshape(input_transposed, [
            input_shape[1] * input_shape[0], self.input_height, self.input_width, input_thickness,self.input_num_atoms])
        input_tensor_reshaped.set_shape((None, self.input_height, self.input_width, input_thickness,self.input_num_atoms))


        if self.upsamp_type == 'resize':
            upsamp = K.resize_images(input_tensor_reshaped, self.scaling, self.scaling, 'channels_last')
            outputs = K.conv3d(upsamp, kernel=self.W, strides=(1, 1), padding=self.padding, data_format='channels_last')
        elif self.upsamp_type == 'subpix':
            conv = K.conv3d(input_tensor_reshaped, kernel=self.W, strides=(1, 1), padding='same',
            ...
            out_thickness = deconv_length(self.input_thickness, self.scaling, self.kernel_size, self.padding)
            output_shape = (batch_size, out_height, out_width, out_thickness ,self.num_capsule * self.num_atoms)

            outputs = K.conv3d_transpose(input_tensor_reshaped, self.W, output_shape, (self.scaling, self.scaling),
                                     padding=self.padding, data_format='channels_last')

        votes_shape = K.shape(outputs)
        _, conv_height, conv_width, conv_thickness,_ = outputs.get_shape()

        votes = K.reshape(outputs, [input_shape[1], input_shape[0], votes_shape[1], votes_shape[2],
                                 self.num_capsule, self.num_atoms])
        votes.set_shape((None, self.input_num_capsule, conv_height.value, conv_width.value,conv_thickness,
                         self.num_capsule, self.num_atoms))
