    ...
    def makePredictions(self, target_class=9):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.testDataLoader):
                ...
                inputs_b, inputs_c, inputs_x, inputs_y, inputs_z = inputs.shape

                inputs_z_new = copy.deepcopy(inputs_z)
                while inputs_z_new % 8 != 0:
                    inputs_z_new -= 1

                inputs = F.interpolate(inputs, size=(inputs_x, inputs_y, inputs_z_new), mode='trilinear', align_corners=True)
                inputs_atlas = F.interpolate(inputs_atlas, size=(inputs_x, inputs_y, inputs_z_new), mode='trilinear', align_corners=True)
                inputs_atlas_mask = F.interpolate(inputs_atlas_mask, size=(inputs_x, inputs_y, inputs_z_new), mode='nearest')
                labels = F.interpolate(labels, size=(inputs_x, inputs_y, inputs_z_new), mode='nearest')
                ...
                outputs_atlas = F.interpolate(outputs_atlas, size=(inputs_x, inputs_y, inputs_z),
                                              mode='trilinear', align_corners=True)
                outputs_atlas_mask = F.interpolate(outputs_atlas_mask, size=(inputs_x, inputs_y, inputs_z),
                                                   mode='nearest')
                outputs_atlas_affine = F.interpolate(outputs_atlas_affine, size=(inputs_x, inputs_y, inputs_z),
                                                     mode='trilinear', align_corners=True)
                outputs_atlas_mask_affine = F.interpolate(outputs_atlas_mask_affine,
                                                          size=(inputs_x, inputs_y, inputs_z),
                                                          mode='nearest')
    ...
    def train(self):
        ...
        while epoch < expConfig.EPOCHS and epoch <= self.bestMovingAvgEpoch + self.EARLY_STOPPING_AFTER_EPOCHS:
            ...
            for i, data in enumerate(self.trainDataLoader):
                ...
                inputs = F.interpolate(inputs, size=(256, 256, inputs_z_new), mode='trilinear', align_corners=True)
                inputs_atlas = F.interpolate(inputs_atlas, size=(256, 256, inputs_z_new), mode='trilinear', align_corners=True)
                inputs_atlas_mask = F.interpolate(inputs_atlas_mask, size=(256, 256, inputs_z_new), mode='nearest')
                labels = F.interpolate(labels, size=(256, 256, inputs_z_new), mode='nearest')
                ...
                loss_dice, loss_mse = expConfig.loss([outputs_atlas,
                                                      outputs_atlas_mask,
                                                      outputs_atlas_affine,
                                                      outputs_atlas_mask_affine],
                                                     [inputs, labels])
