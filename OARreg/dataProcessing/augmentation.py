...
def augment3DImage(img, lbl, defaultLabelValues, nnAug, do_rotate=True, rotDegrees=20, do_scale=True, scaleFactor=1.1, do_flip=True, do_elasticAug=True, sigma=10, do_intensityShift=True, maxIntensityShift=0.1):
    ...
    if do_intensityShift:
        for i in range(cSize): #number of channels
...
def augment3DImageEdgeTemp(img, lbl, edge, temp_edge,defaultLabelValues, nnAug, do_rotate=True, rotDegrees=20, do_scale=True, scaleFactor=1.1, do_flip=True, do_elasticAug=True, sigma=10, do_intensityShift=True, maxIntensityShift=0.1):
    ...
    if do_flip:
        for i in range(1):
            if np.random.random() < 0.5:
                ...
                lbl = np.flip(lbl, axis=i)
                edge = np.flip(edge, axis=i)
                temp_edge = np.flip(temp_edge, axis=i)
    ...
    return img.copy(), lbl.copy(), edge.copy(), temp_edge.copy() #pytorch cannot handle negative stride in view

def augment3DImageTemp(img, lbl, temp, temp_lbl, defaultLabelValues, nnAug, do_rotate=True, rotDegrees=20, do_scale=True, scaleFactor=1.1, do_flip=True, do_elasticAug=True, sigma=10, do_intensityShift=True, maxIntensityShift=0.1):
    '''
    Function for augmentation of a 3D image. It will transform the image and corresponding labels
    by a number of optional transformations.
    :param img: A numpy array of shape [X, Y, Z, nChannels]
    ...
    if do_rotate:
        ...
        for z in range(zSize):
            img[:, :, z, :] = utils.rotate_image(img[:, :, z, :], random_angle)
            lbl[:, :, z, :] = utils.rotate_image(lbl[:, :, z, :], random_angle, interpolationMethod)
            # temp[:, :, z, :] = utils.rotate_image(edge[:, :, z, :], random_angle, interpolationMethod)
    ...
    if do_flip:
        for i in range(1):
            if np.random.random() < 0.5:
                img = np.flip(img, axis=i)
                lbl = np.flip(lbl, axis=i)
                temp = np.flip(temp, axis=i)
                temp_lbl = np.flip(temp_lbl, axis=i)
    ...
    return img.copy(), lbl.copy(), temp.copy(), temp_lbl.copy() #pytorch cannot handle negative stride in view
