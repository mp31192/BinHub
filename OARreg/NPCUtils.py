...
def HuMoment(map):
    if map.is_cuda:
        ...
        device = torch.device("cpu")
    matrix_index = torch.ones([map.shape[1], map.shape[2], 2])
    matrix_index = matrix_index.to(device)

    matrix_index[:, :, 0] = torch.mul(matrix_index[:, :, 0],
                                      torch.reshape(torch.range(0, map.shape[1] - 1), [map.shape[1], 1, 1]).to(device))
    matrix_index[:, :, 1] = torch.mul(matrix_index[:, :, 1],
                                      torch.reshape(torch.range(0, map.shape[2] - 1), [1, map.shape[2], 1]).to(device))
    ...
    u00 = m00
    n20 = torch.sum(map * ((matrix_index[:, :, 0] - x0) ** 2)) / u00
    n02 = torch.sum(map * ((matrix_index[:, :, 1] - y0) ** 2)) / u00
    n11 = torch.sum(map * (matrix_index[:, :, 1] - y0) * (matrix_index[:, :, 0] - x0)) / u00
    n12 = torch.sum(map * (matrix_index[:, :, 0] - x0) * ((matrix_index[:, :, 1] - y0) ** 2)) / (u00 ** 1.5)
    n21 = torch.sum(map * ((matrix_index[:, :, 0] - x0) ** 2) * (matrix_index[:, :, 1] - y0)) / (u00 ** 1.5)
    n30 = torch.sum(map * ((matrix_index[:, :, 0] - x0) ** 3)) / (u00 ** 1.5)
    n03 = torch.sum(map * ((matrix_index[:, :, 1] - y0) ** 3)) / (u00 ** 1.5)
    ...
    h1 = torch.sign(h1) * torch.log10(torch.abs(h1))
    h2 = torch.sign(h2) * torch.log10(torch.abs(h2))
    h3 = torch.sign(h3) * torch.log10(torch.abs(h3))
    h4 = torch.sign(h4) * torch.log10(torch.abs(h4))
    h5 = torch.sign(h5) * torch.log10(torch.abs(h5))
    h6 = torch.sign(h6) * torch.log10(torch.abs(h6))
    h7 = torch.sign(h7) * torch.log10(torch.abs(h7))
