    ...
    def makePredictions(self, target_class=9):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.testDataLoader):
                ...
                outputs_atlas_mask = torch.zeros((inputs.shape[0], 9, inputs.shape[2],
                                                  inputs.shape[3], inputs.shape[4])).to(self.device)

                for sl in range(9):
                    select_channel = sl
                    outputs_atlas_affine, outputs_atlas_mask_affine, outputs_class = \
                        expConfig.net(inputs, inputs_atlas,
                                      inputs_atlas_mask[:, select_channel:select_channel+1, :, :, :],
                                      select_channel)
                    outputs_atlas_mask[:, select_channel, :, :, :] = outputs_atlas_mask_affine[:, 0, :, :, :]

                #
                print("Spend Time:", time.time() - start_time)

                ## affine mask
                fullsize = outputs_atlas_mask
                # binarize output
                ...
                # npResult = result[:, :, :].cpu().numpy()
                # npResult = np.transpose(npResult, [1, 0, 2])
                # path = os.path.join(basePath, "{}_0_input_image.nii.gz".format(pid[0]))
                # utils.save_nii(path, npResult, None, None)
    ...
    def train(self):
        ...
        while epoch < expConfig.EPOCHS and epoch <= self.bestMovingAvgEpoch + self.EARLY_STOPPING_AFTER_EPOCHS:
            ...
            for i, data in enumerate(self.trainDataLoader):
                ...
                resize_ratio[0] = resize_shape[0] / inputs_shape[2]
                resize_ratio[1] = resize_shape[1] / inputs_shape[3]
                resize_ratio[2] = resize_shape[2] / inputs_shape[4]
                ...
                for sss in range(3):
                    if resize_arg == sss:
                        ...
                        if final_resize_shape[sss] > resize_shape[sss]:
                            final_resize_shape[sss] = resize_shape[sss]
                ...
                pad_z1 = np.maximum(int((resize_shape[2] - final_resize_shape[2]) / 2), 0)
                pad_z2 = np.maximum(int((resize_shape[2] - final_resize_shape[2]) - pad_z1), 0)
                pad_y1 = np.maximum(int((resize_shape[1] - final_resize_shape[1]) / 2), 0)
                pad_y2 = np.maximum(int((resize_shape[1] - final_resize_shape[1]) - pad_y1), 0)
                pad_x1 = np.maximum(int((resize_shape[0] - final_resize_shape[0]) / 2), 0)
                pad_x2 = np.maximum(int((resize_shape[0] - final_resize_shape[0]) - pad_x1), 0)
                ...
                labels = F.interpolate(labels, size=(28, 20, 12), mode='nearest')
                ...
                for sl in range(9):
                    select_channel = sl
                    if torch.sum(labels[:, select_channel:select_channel+1, :, :, :]).item() == 0:
                        continue
                    outputs_mask, outputs_class = \
                        expConfig.net(inputs, inputs_atlas,
                                      inputs_atlas_mask[:, select_channel:select_channel+1, :, :, :],
                                      select_channel)
                    ...
                    loss_dice = expConfig.loss([outputs_mask[:, select_channel:select_channel+1, :, :, :]],
                                               [labels[:, select_channel:select_channel+1, :, :, :]])

                    print("Seg Loss:", loss_dice.item(), " Cls Loss:", class_loss.item())

                    loss = loss_dice + class_loss * 0.5
    ...
    def validate(self, epoch, target_class = 9):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(self.valDataLoader):
                ...
                outputs_atlas_mask = torch.zeros((inputs.shape[0], 9, 28, 20, 12)).to(self.device)

                for sl in range(9):
                    select_channel = sl
                    if torch.sum(inputs_atlas_mask[:, select_channel:select_channel+1, :, :, :]).item() == 0:
                        continue
                    outputs_mask, outputs_class = \
                        expConfig.net(inputs, inputs_atlas,
                                      inputs_atlas_mask[:, select_channel:select_channel+1, :, :, :],
                                      select_channel)
                    outputs_atlas_mask[:, select_channel, :, :, :] = outputs_mask[:, select_channel, :, :, :]
