    ...
    def makePredictions(self, target_class=9):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.testDataLoader):
                ...
                outputs_atlas_mask = torch.zeros(net_inputs_list[0].shape[0], 9,
                                                 net_inputs_list[0].shape[2], net_inputs_list[0].shape[3],
                                                 net_inputs_list[0].shape[4]).to(self.device)

                for sl in range(len(net_inputs_list)):

                    if net_inputs_list[sl] is None:
                        continue
                    ...
                    # with autocast():
                    outputs_atlas_affine, outputs_atlas_mask_affine, outputs_atlas_box_affine, outputs_class = \
                        expConfig.net(inputs, inputs_atlas,
                                      inputs_atlas_mask,
                                      inputs_atlas_bbox)

                    outputs_atlas_mask[:, sl, :, :, :] = outputs_atlas_mask_affine[:, 0, :, :, :]
            ...
            for i, data in enumerate(self.trainDataLoader):
                ...
                for sl in range(len(net_labels_list)):
                    ...
                    with autocast():
                        ...
                        loss_dice, loss_mse, loss_dist = expConfig.loss([outputs_atlas_affine, outputs_atlas_mask_affine,
                                                                         outputs_atlas_box_affine],
                                                                        [inputs, labels, labels_box])

                    print("Seg Loss:", loss_dice.item(), " MSE Loss:", loss_mse.item(),
                          " Dist Loss:", loss_dist.item(), " Cls Loss:", class_loss.item())

                    loss = loss_dice + loss_dist * 5 + loss_mse * 0.5 + class_loss * 0.25
                    ...
                    # print("Time each step ", time.time() - startTime_test)
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(self.valDataLoader):
                ...
                for sl in range(len(net_labels_list)):
                    ...
                    _, _, outputs_atlas_box_affine, _ = \
                        expConfig.net(inputs, inputs_atlas,
                                      inputs_atlas_mask,
                                      inputs_atlas_bbox)

                    outputs_atlas_mask[:, sl, :, :, :] = outputs_atlas_box_affine[:, 0, :, :, :]
                    labels_mid[:, sl, :, :, :] = labels_box[:, 0, :, :, :]
