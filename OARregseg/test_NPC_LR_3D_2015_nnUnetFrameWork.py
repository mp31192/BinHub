...
def resize_image_itk(itkimage, itktargetimage, resamplemethod=sitk.sitkNearestNeighbor):
    """
    ...
    newSize = itktargetimage.GetSize()
...
predict_path = '/media/root/01456da2-1f1d-4b67-810e-b9cd3341133d/NPC_MICCAI_2015_original_data/HaN_2015_crop/nnUnet_dataset/nnUnet_raw/nnUNet_raw_data/Task315_OAR2015/outputsTs'
# mask_path = '/media/root/01456da2-1f1d-4b67-810e-b9cd3341133d/2019_MICCAI_challenge_NPC/HaN_OAR_crop/test'
mask_path = '/media/root/01456da2-1f1d-4b67-810e-b9cd3341133d/NPC_MICCAI_2015_original_data/HaN_2015_crop/nnUnet_dataset/nnUnet_raw/nnUNet_raw_data/Task315_OAR2015/labelsTs'
...
for fl in file_list:
    predict_name = fl
    if '.nii.gz' not in predict_name:
        continue
    print(predict_name)
    R += 1
    patient_id = predict_name.split('.')[0]
    ...
    predict_result_path = os.path.join(predict_path, patient_id + '.nii.gz')
    mask_fullpath = os.path.join(mask_path, patient_id + '.nii.gz')
    labels = sitk.ReadImage(mask_fullpath)
    predict_result = sitk.ReadImage(predict_result_path)
    # _, predict_result = resize_image_itk(predict_result, labels, resamplemethod=sitk.sitkNearestNeighbor)
    # sitk.WriteImage(predict_result, os.path.join(predict_path, patient_id + '_0_result_resample.nii.gz'))
    labels = sitk.GetArrayFromImage(labels)
    predict_result = sitk.GetArrayFromImage(predict_result)
